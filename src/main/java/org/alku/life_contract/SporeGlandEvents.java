...
public class SporeGlandEvents {
    ...
    public static void onPlayerAttack(AttackEntityEvent event) {
        ...
        if (sporeGland.isPresent() && RANDOM.nextFloat() < Config.sporeChance) {
            applySlowInfection(livingTarget);
    ...
    private static void applySlowInfection(LivingEntity target) {
        int duration = Config.infectionDuration;
        CompoundTag data = target.getPersistentData();
        data.putBoolean(TAG_SLOW_INFECTION, true);
        data.putInt(TAG_SPORE_SPAWN_TIME, target.tickCount + duration);

        target.addEffect(new MobEffectInstance(Life_contract.SLOW_INFECTION.get(), duration, 0, false, true));
    }
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (Config.eliteMobs.contains(entityId)) {
